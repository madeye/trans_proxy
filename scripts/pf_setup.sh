...
Examples:
  $0 en0                                    # redirect all TCP on en0 (except SSH) to port 8443
...
if [ -n "$PORTS" ]; then
    ...
    if [ -n "$IFACE_IP" ]; then
        SSH_BYPASS="pass in quick on ${IFACE} proto tcp from any to ${IFACE_IP} port 22
"
    else
        SSH_BYPASS=""
    fi
...
echo ""
echo "Done."
echo "  Gateway IP:  ${IFACE_IP:-<unknown>} (${IFACE})"
if [ -n "$PORTS" ]; then
